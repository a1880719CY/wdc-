...
const vueinst = new Vue({
    ...
    methods: {
        setTopMenuItem(index) {
            ...
            this.top_menu_hover = true; // Keep submenu visible
        },
        clearTopMenuItem() {
            // Check if the cursor is still on the submenu
            setTimeout(() => {
                if (!this.top_menu_hover) {
                    this.top_menu_item = -1;
                }
            }, 200); // Delay to allow moving cursor from menu to submenu
        },
        setTopMenuHover(value) {
            this.top_menu_hover = value;
        },
        addComment() {
